--[[
	...
	- 🏃 Movimiento: WalkSpeed, JumpPower, Gravity, Salto Infinito en el Aire (Tecla ESPACIO), Salto de Desplazamiento Vertical (Tecla F), Noclip, Fly Mode, Impulse Dash.
...
print("[Admin Panel] Iniciando carga...")

local LocalPlayer = Players.LocalPlayer or Players:GetPropertyChangedSignal("LocalPlayer"):Wait() or Players.LocalPlayer
local Camera = Workspace.CurrentCamera or Workspace:WaitForChild("Camera")
local Mouse = LocalPlayer:GetMouse()

print("[Admin Panel] Variables iniciales cargadas. LocalPlayer:", LocalPlayer.Name)

-- =========================================================
-- CARGA DE SECURE.LUA Y PROTECCIÓN BÁSICA
-- =========================================================
local Secure
print("[Admin Panel] Intentando cargar secure.lua...")
local successSecure, resultSecure = pcall(function()
	if isfile and isfile("secure.lua") then
		return loadstring(readfile("secure.lua"))()
	end
	return loadstring(game:HttpGet("https://pastebin.com/raw/wC2f6Xtp"))()
end)

if successSecure and resultSecure then
	Secure = resultSecure
	print("[Admin Panel] secure.lua cargado exitosamente.")
else
	warn("[Admin Panel] No se pudo cargar secure.lua. Ejecutando en Modo Seguro. Error:", tostring(resultSecure))
end
...
local State = {
	...
	-- Movimiento avanzado & SISTEMA DE DOBLE SALTO
	...
	ForwardJumpDistance = 80,           -- Fuerza / Distancia de impulso
...
-- =========================================================
-- SISTEMA DE PERSISTENCIA Y SERIALIZACIÓN (METADATOS)
-- =========================================================
local ConfigName = "CombatAdminState"
...
local function saveState()
	if not Secure or not Secure.Config then return end
	local data = {}
	for k, v in pairs(State) do
		if typeof(v) == "EnumItem" then
			data[k] = {Type = "KeyCode", Name = v.Name}
		elseif typeof(v) == "Color3" then
			data[k] = {Type = "Color3", Value = serializeColor(v)}
		elseif type(v) ~= "function" and type(v) ~= "userdata" then
			data[k] = v
		end
	end
	Secure.Config:Save(ConfigName, data)
end

local function loadState()
	if not Secure or not Secure.Config then return end
	local data = Secure.Config:Load(ConfigName)
	if data then
		for k, v in pairs(data) do
			if type(v) == "table" and v.Type == "KeyCode" then
				pcall(function() State[k] = Enum.KeyCode[v.Name] end)
			elseif type(v) == "table" and v.Type == "Color3" then
				pcall(function() State[k] = deserializeColor(v.Value) end)
			else
				State[k] = v
			end
		end
...
-- =========================================================
print("[Admin Panel] Iniciando creación de GUI (Estilo test_secure.lua)...")
...
if CoreGui then
	local oldCoreGui = CoreGui:FindFirstChild("CombatAdminGui")
	if oldCoreGui then pcall(function() oldCoreGui:Destroy() end) end
end
if gethui then
	pcall(function()
		local oldHui = gethui():FindFirstChild("CombatAdminGui")
...
-- Protección básica de GUI (Estilo test_secure.lua)
pcall(function() if syn and syn.protect_gui then syn.protect_gui(screenGui) end end)

-- Asignación de Parent usando la cascada de test_secure.lua
...
if Secure and Secure.GUI then
	pcall(function() Secure.GUI:BlockDetectionText(LocalPlayer) end)
end

print("[Admin Panel] GUI Parent final:", screenGui.Parent and screenGui.Parent.Name or "NIL")

-- Activar Protecciones si Secure está disponible
if Secure then
	if Secure.Environment then
		pcall(function() Secure.Environment:ProtectPlayer(LocalPlayer) end)
		pcall(function() Secure.Environment:SpoofCallStack() end)
	end
	if Secure.Presence then
		pcall(function() Secure.Presence:EnableAntiAFK() end)
	end
end
...
-- =========================================================
-- FUNCIÓN DEL MODO VUELO (FLY MODE)
-- =========================================================
...
-- =========================================================
-- PESTAÑA 1: MOVIMIENTO & SISTEMA DE DOBLE SALTO
-- =========================================================
createInputRow(movementPage, "Velocidad de Caminata (Speed):", State.WalkSpeed, 0, 500, function(val)
	State.WalkSpeed = val
	local char = LocalPlayer.Character
	if char and char:FindFirstChildOfClass("Humanoid") then
...
createActionButton(movementPage, "⚡ Impulso / Dash de Desarrollador", PALETTE.Primary, function()
	local char = LocalPlayer.Character
...
table.insert(Connections, UserInputService.InputBegan:Connect(function(input, gpe)
	...
	-- 5. Captura de Clics para Click-to-TP y Click Select (Fuera de la GUI del Admin)
	...
	-- 6. Grabación de Macro: Filtra Atajos y Teclas de Control para No Contaminar la Macro
	...
	-- 7. Teclas rápidas simples (Sprint)
	if not gpe then
...
table.insert(Connections, RunService.RenderStepped:Connect(function()
	...
	if char and char:FindFirstChild("HumanoidRootPart") then
		...
		telemetryBar.Text = string.format("FPS: %d | Panel (Ctrl+%s) | Salto Aire (Space) | Salto Vert (%s) | AutoClick (Ctrl+%s)", 
			fps, State.ToggleKey.Name, State.JumpKey.Name, State.AutoClickKey.Name)
	end

	-- Procesar Auto Clicker y Fly Mode
	processAutoClick()
	processFlyMode()
